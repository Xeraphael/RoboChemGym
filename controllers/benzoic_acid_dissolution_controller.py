...
class BenzoicAcidDissolutionController(BaseController):
    ...
    def step(self, state: Dict[str, Any]) -> Tuple[Any, bool, bool]:
        ...
        if self._event == 0:
            ...
            if not self.pick_controller.is_done():
                ...
                action = self.pick_controller.forward(
                    ...
                    gripper_distances=0.012  # ErlenmeyerFlask_Solid 特殊设置
                )
        ...
        # 阶段1：将 ErlenmeyerFlask_Solid1 放置到 HeatingPlate
        elif self._event == 1:
            ...
            if not self.place_controller.is_done():
                ...
                action = self.place_controller.forward(
                    ...
                    place_offset_z=0.12  # ErlenmeyerFlask_Solid 特殊设置
                )
        ...
        # 阶段2：按动加热板按钮启动搅拌
            ...
            if not self.press_controller.is_done():
                target_name = "HeatingPlate"
                pos = state.get(f"{target_name}_press_position")
                if pos is None:
                    return None, False, False
        ...
        # 阶段3：抓取 ErlenmeyerFlask_Liquid1
        elif self._event == 3:
            self.current_action_type = "pick"
            if not self.pick_controller.is_done():
                ...
                action = self.pick_controller.forward(
                    ...
                    gripper_position=state['gripper_position'],
                    end_effector_orientation=R.from_euler('xyz', np.radians([0, 90, 30])).as_quat(),
                    gripper_distances=0.008  # ErlenmeyerFlask_Liquid 特殊设置
                )
                self.gripper_control.update_grasped_object_position()
        ...
        # 阶段4：将 ErlenmeyerFlask_Liquid1 倒入 ErlenmeyerFlask_Solid1
        elif self._event == 4:
        ...
        # 阶段5：将 ErlenmeyerFlask_Liquid1 放回 TargetPlatform1
        elif self._event == 5:
            self.current_action_type = "place"
            if not self.place_controller.is_done():
                target_name = "TargetPlatform1"
                pos = state.get(f"{target_name}_place_position", state.get(f"{target_name}_position"))
                if pos is None:
                    return None, False, False
                ...
                action = self.place_controller.forward(
                    ...
                    gripper_position=state['gripper_position'],
                    end_effector_orientation=R.from_euler('xyz', np.radians([0, 90, 30])).as_quat(),
                    place_offset_z=0.08  # ErlenmeyerFlask_Liquid 特殊设置
                )
