...
from controllers.joint_action_safety import bound_joint_position_step
...
class PlaceController(BaseController):
    ...
    def forward(
        ...
        object_position: typing.Optional[np.ndarray] = None,
        ...
        if self._event == 0:
            ...
            target_joint_positions = bound_joint_position_step(
                self._cspace_controller.forward(
            ...
            if object_position is not None:
                xy_distance = np.linalg.norm(place_position[:2] - object_position[:2])
            ...
            if gripper_position is not None and object_position is not None:
                object_xy_distance = np.linalg.norm(
                    place_position[:2] - object_position[:2]
                )
                gripper_z_distance = abs(
                    self.target_position[2] - gripper_position[2]
                )
                if object_xy_distance < self._position_threshold and gripper_z_distance < 0.03:
                    self._event += 1
                    self._t = 0
        elif self._event == 2:
        ...
        # Motion events advance only after reaching their spatial goal. The
        # plan executor owns the real timeout.
        if self._event in (2, 3, 5):
