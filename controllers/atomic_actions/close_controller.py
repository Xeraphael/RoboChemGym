...
class CloseController(BaseController):
    ...
    def forward(
        ...
        if push_distance is not None:
            push_distance = push_distance / get_stage_units()
    ...
    def reset(self, furniture_type: typing.Optional[str] = None) -> None:
        """Reset controller state"""
        BaseController.reset(self)
        self._event = 0
        self._t = 0
        if furniture_type is not None and furniture_type != self.furniture_type:
            self.furniture_type = furniture_type
            self._events_dt = [0.0005, 0.002, 0.05, 0.008] if self.furniture_type == "drawer" else [0.0025, 0.005, 0.005]
        self.position_rotation_interp_iter = None
        self.init_handle_position = None
    ...
    def rotate_quaternion_around_x(self, q, angle_deg):
        ...
        q_xyzw = np.asarray(q)[[1, 2, 3, 0]]
        r = R.from_quat(q_xyzw)
        r_rot = R.from_quat(q_rot)
        r_new = r * r_rot
        result_xyzw = r_new.as_quat()
