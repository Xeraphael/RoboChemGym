...
class PickController(BaseController):
    ...
    def __init__(
        ...
        self._event4_bound_object_prim_path = None
        self._grasp_target_offset = None
    ...
    def forward(
        ...
        object_prim_path: typing.Optional[str] = None,
        pick_z_offset: typing.Optional[float] = None,
    ) -> ArticulationAction:
    ...
    def _execute_phase(self, picking_position, end_effector_orientation, current_joint_positions, object_name, gripper_control, gripper_position, gripper_distances, object_prim_path=None, pick_z_offset=None):
        ...
        if self._event == 0:
            ...
            picking_position[2] += self.object_size[2] + self.pre_offset_z / get_stage_units()
            ...
            raw_picking_position = picking_position.copy()
            if pick_z_offset is None:
                pick_z_offset = self.get_pickz_offset(object_name)
            picking_position[2] += pick_z_offset / get_stage_units()
            self._grasp_target_offset = picking_position - raw_picking_position
            target_joint_positions = self._cspace_controller.forward(
                target_end_effector_position=picking_position,
                target_end_effector_orientation=end_effector_orientation
            )
            ...
            if object_prim_path is not None:
                binding_position = (
                    picking_position
                    if self._grasp_target_offset is None
                    else picking_position + self._grasp_target_offset
                )
                binding_distance = np.linalg.norm(
                    gripper_position - binding_position
                )
                binding_threshold = 0.03 / get_stage_units()
                if (
                    not np.isfinite(binding_distance)
                    or binding_distance > binding_threshold
                ):
                    self._event = 2
                    self._t = 0
                    return self._cspace_controller.forward(
                        target_end_effector_position=binding_position,
                        target_end_effector_orientation=end_effector_orientation,
                    )
            target_joint_positions = [None] * current_joint_positions.shape[0]
            if gripper_distances is None:
                gripper_distances = self.get_gripper_distance(object_name)
            gripper_distance_stage = gripper_distances / get_stage_units()
            target_joint_positions[7] = gripper_distance_stage
            target_joint_positions[8] = gripper_distance_stage
            target_joint_positions = ArticulationAction(joint_positions=target_joint_positions)
            self.target_position = (
                picking_position.copy()
                if self._grasp_target_offset is None
                else picking_position + self._grasp_target_offset
            )
            self.target_position[2] += self.after_offset_z / get_stage_units()

            # 将对象刚性绑定到夹爪：原有玻璃棒逻辑 + 扩展到锥形瓶
            if object_prim_path is not None:
                if self._event4_bound_object_prim_path != object_prim_path:
                    gripper_control.add_object_to_gripper(
                        object_prim_path,
                        "/World/Franka/panda_hand/tool_center",
                        resolve_rigid_body=True,
                    )
                    self._event4_bound_object_prim_path = object_prim_path
            elif "glass" in object_name:
                # 玻璃棒：绑定到对应刚体 prim
                gripper_control.add_object_to_gripper(
                    "/World/glass_rod/Cylinder", "/World/Franka/panda_hand/tool_center"
                )
            elif "ErlenmeyerFlask" in object_name:
                # 锥形瓶：绑定到 /World/LabScene/<name> 下的 Xform 根节点
                flask_path = f"/World/LabScene/{object_name}"
                gripper_control.add_object_to_gripper(
                    flask_path, "/World/Franka/panda_hand/tool_center"
                )
            elif "Beaker" in object_name:
                # 烧杯：绑定到 /World/LabScene/<name> 下的 Xform 根节点
                beaker_path = f"/World/LabScene/{object_name}"
                gripper_control.add_object_to_gripper(
                    beaker_path, "/World/Franka/panda_hand/tool_center"
                )
            return target_joint_positions
            ...
            # event 6：继续保持抬升目标位置，避免关节驱动失效导致腕部缓慢旋转
    ...
    def get_pickz_offset(self, item_name):
        ...
        for key in offsets:
            if key == item_name.lower() or key == item_name:
                return offsets[key]
