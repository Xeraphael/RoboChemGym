...
from pipeline.randomization import SceneRandomizer
...
class BaseTask(ABC):
    ...
    def __init__(self, cfg, world, stage, robot):
        ...
        self.current_randomization = None
        self.scene_randomizer = None
        self.setup_cameras()
        self.setup_objects()
        self.setup_materials()
        if hasattr(cfg, "randomization"):
            randomization = RandomizationConfig.model_validate(
                OmegaConf.to_container(cfg.randomization, resolve=True)
            )
            self.scene_randomizer = SceneRandomizer(
                randomization, self.stage, self.object_utils
            )
    ...
    def reset(self) -> None:
        ...
        if self.scene_randomizer is not None:
            self.current_randomization = self.scene_randomizer.apply(
                self.episode_index
            )
    ...
    def get_camera_calibration(self) -> list[dict[str, Any]]:
        calibrations = []
        for camera, config in zip(self.cameras, self.cfg.cameras):
            ...
            calibrations.append(
                {
                    ...
                    "world_position": np.asarray(position).tolist(),
                    "world_orientation": np.asarray(orientation).tolist(),
                }
            )
        return calibrations
